...
def update_tryhackme(username='gvbytes'):
    ...
    if not completed_rooms:
        completed_rooms = [{'title': 'Tutorial', 'status': 'completed'}, {'title': 'Offensive Security Intro', 'status': 'completed'}, {'title': 'Defensive Security Intro', 'status': 'completed'}, {'title': 'Careers in Cyber', 'status': 'completed'}]
        rooms_completed = 4
    badge_path = None
    ...
    result = {'username': username, 'profile_url': f'https://tryhackme.com/p/{username}', 'rank': rank, 'rooms_completed': rooms_completed, 'badges': badges, 'completed_rooms': completed_rooms, 'badge_image_path': badge_path, 'last_updated': datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')}
...
def update_leetcode(username='gvbytes'):
    ...
    result = {'username': username, 'profile_url': f'https://leetcode.com/u/{username}/', 'total_solved': total_solved, 'easy_solved': easy_solved, 'medium_solved': medium_solved, 'hard_solved': hard_solved, 'total_easy': total_easy, 'total_medium': total_medium, 'total_hard': total_hard, 'ranking': ranking, 'last_updated': datetime.now(timezone.utc).strftime('%Y-%m-%dT%H:%M:%SZ')}
    with open(outfile, 'w') as f:
        json.dump(result, f, indent=2)
        f.write('\n')
