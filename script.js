...
(() => {
	...
	function init3DScene() {
		...
		const mWater = new THREE.MeshBasicMaterial({
			color: 0x00d4ff,
			transparent: true,
			opacity: 0.85,
		});
		...
		function animate(timestamp) {
			...
			orbitRig.rotation.y = elapsedTime * 0.3;
			cloudRig.rotation.y += 0.01 * delta * 60;
	...
	class PhysicsBody {
		constructor(element, type, container) {
			...
			this.type = type;
			this.container =
				container || element.closest("section, footer, #hero") || document.body;
			this.x = 0;
			this.y = 0;
			this.vx = 0;
			this.vy = 0;
	...
	function preparePhysicsBodies() {
		...
		document
			.querySelectorAll(".about-text p, .section-subtitle, .footer-bottom p")
			.forEach((p) => {
				splitText(p, "word");
			});
			...
			btn.addEventListener("click", () => {
				...
				if (mode === "explode") {
					...
					physicsBodies.forEach((p) => {
						...
						const force =
							(isMobile ? 6 : 12) + Math.random() * (isMobile ? 5 : 10);
						p.vx =
							(dx / dist) * force + (Math.random() - 0.5) * (isMobile ? 2 : 4);
	...
	function createProjectCard(project, index) {
		...
		card.href =
			project.html_url || `https://github.com/${GH_USER}/${project.name}`;
		card.target = "_blank";
	...
	async function fetchGitHubRepos() {
		...
		try {
			...
			const filtered = repos.filter(
				(r) => !r.fork && !EXCLUDE_REPOS.includes(r.name),
			);
			try {
	...
	async function renderProjects() {
		...
		container.innerHTML =
			'<p class="pixel" style="text-align:center;padding:40px 0;color:var(--muted);font-size:1.3rem;">Loading public projects from GitHub…</p>';
		...
		if (!repos || repos.length === 0) {
			container.innerHTML =
				'<p class="pixel" style="text-align:center;font-size:1.3rem;color:var(--muted);">Could not load projects. <a href="https://github.com/gvbytes" target="_blank" style="color:var(--accent);">View on GitHub →</a></p>';
			return;
		}
		...
		const vulnMapped = [
			"srm-secure-browser-vulnerabilities",
			"srm-secure-browser-review",
		];
		const otherMapped = [
			"del-and-bits",
			"del-bits",
